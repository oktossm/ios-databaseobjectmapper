...
/// `DatabaseEncoder` facilitates the encoding of `Encodable` values into Dictionary.
open class DatabaseEncoder {
    // MARK: Options

    /// The strategy to use for encoding custom values.
    public enum ValueEncodingStrategy {
        /// Use default logic
        case deferredToValue

        /// Encode value itself if given closure returns true. Otherwise default logic used.
        case asIs(ValueAsIsStrategyHelper)
    }


    /// The strategy to use for encoding `Date` values.
    public enum DateEncodingStrategy {
        /// Just store `Date` and don't decode to another type.
        case asDate
    ...
    public enum DataEncodingStrategy {
        /// Just store the `Data` and don't decode to an other type.
        case asData
    ...
    /// The strategy to use in encoding custom values. Defaults to `.deferredToValue`.
    open var valueEncodingStrategy: ValueEncodingStrategy = .deferredToValue
    ...
    fileprivate struct _Options {
        let valueEncodingStrategy: ValueEncodingStrategy
    ...
    fileprivate var options: _Options {
        return _Options(valueEncodingStrategy: valueEncodingStrategy,
...
fileprivate class _DictionaryEncoder: Encoder {
    ...
    fileprivate let options: DatabaseEncoder._Options
    ...
    fileprivate init(options: DatabaseEncoder._Options, codingPath: [CodingKey] = []) {
    ...
    public func container<Key>(keyedBy: Key.Type) -> KeyedEncodingContainer<Key> {
        ...
        if self.canEncodeNewValue {
            ...
            guard let container = self.storage.containers.last as? BoxedDictionary<String, Any?> else {
    ...
    public func unkeyedContainer() -> UnkeyedEncodingContainer {
        ...
        let topContainer: BoxedArray<Any?>
        if self.canEncodeNewValue {
            // We haven't yet pushed a container at this level; do so here.
            topContainer = self.storage.pushUnkeyedContainer()
        } else {
            guard let container = self.storage.containers.last as? BoxedArray<Any?> else {
...
fileprivate struct _DictionaryEncodingStorage {
    // MARK: Properties
    /// The container stack.
    /// By default elements may be any one of the Dictionary types (nil, Number, String, Array, Dictionary).
    private(set) fileprivate var containers = BoxedArray<Any?>()
    ...
    fileprivate mutating func pushKeyedContainer() -> BoxedDictionary<String, Any?> {
        let dictionary = BoxedDictionary<String, Any?>()
        self.containers.append(dictionary)
        return dictionary
    }

    fileprivate mutating func pushUnkeyedContainer() -> BoxedArray<Any?> {
        let array = BoxedArray<Any?>()
...
fileprivate struct DictionaryCodingKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
    ...
    private let container: BoxedDictionary<String, Any?>
    ...
    fileprivate init(referencing encoder: _DictionaryEncoder, codingPath: [CodingKey], wrapping container: BoxedDictionary<String, Any?>) {
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = DatabaseEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
            return DatabaseCodingKey(stringValue: newKeyString, intValue: key.intValue)
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> {
        ...
        let container = DictionaryCodingKeyedEncodingContainer<NestedKey>(referencing: self.encoder,
...
fileprivate struct _DictionaryUnkeyedEncodingContainer: UnkeyedEncodingContainer {
    ...
    /// A reference to the container we're writing to.
    private let container: BoxedArray<Any?>
    ...
    // MARK: - Initialization
    /// Initializes `self` with the given references.
    fileprivate init(referencing encoder: _DictionaryEncoder, codingPath: [CodingKey], wrapping container: BoxedArray<Any?>) {
        self.encoder = encoder
        self.codingPath = codingPath
        self.container = container
    }
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type) -> KeyedEncodingContainer<NestedKey> {
        self.codingPath.append(DatabaseCodingKey(index: self.count))
        defer { self.codingPath.removeLast() }
...
extension _DictionaryEncoder {
    ...
    fileprivate func box(_ value: Int) -> Any { return value }

    fileprivate func box(_ value: Int8) -> Any { return value }

    fileprivate func box(_ value: Int16) -> Any { return value }

    fileprivate func box(_ value: Int32) -> Any { return value }

    fileprivate func box(_ value: Int64) -> Any { return value }

    fileprivate func box(_ value: UInt) -> Any { return value }

    fileprivate func box(_ value: UInt8) -> Any { return value }

    fileprivate func box(_ value: UInt16) -> Any { return value }

    fileprivate func box(_ value: UInt32) -> Any { return value }

    fileprivate func box(_ value: UInt64) -> Any { return value }
    ...
    fileprivate func box<T: Encodable>(_ value: T) throws -> Any {
        switch self.options.valueEncodingStrategy {
        case .deferredToValue:
            break
        case .asIs(let helper):
            var anyValue: Any? = value
            if helper.useValueAsIs(&anyValue, ofType: T.self) {
                return anyValue ?? [String: Any?]()
            }
        }
        return try self.box_(value) ?? [String: Any?]()
    }
...
fileprivate class _DictionaryReferencingEncoder: _DictionaryEncoder {
    ...
    private enum Reference {
        ...
        case array(BoxedArray<Any?>, Int)

        /// Referencing a specific key in a dictionary container.
        case dictionary(BoxedDictionary<String, Any?>, String)
    }
    ...
    fileprivate init(referencing encoder: _DictionaryEncoder, at index: Int, wrapping array: BoxedArray<Any?>) {
        self.encoder = encoder
        self.reference = .array(array, index)
        super.init(options: encoder.options, codingPath: encoder.codingPath)

        self.codingPath.append(DatabaseCodingKey(index: index))
    }

    /// Initializes `self` by referencing the given dictionary container in the given encoder.
    fileprivate init(referencing encoder: _DictionaryEncoder,
                     key: CodingKey, convertedKey: CodingKey, wrapping dictionary: BoxedDictionary<String, Any?>) {
        self.encoder = encoder
    ...
    deinit {
        ...
        case 0: value = BoxedDictionary<String, Any?>()
