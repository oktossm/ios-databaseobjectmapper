...
// Database Decoder
//===----------------------------------------------------------------------===//
/// `DatabaseDecoder` facilitates the decoding of Dictionary into semantic `Decodable` types.
open class DatabaseDecoder {
    // MARK: Options
    /// The strategy to use for decoding custom values.
    public enum ValueDecodingStrategy {
        /// Use default logic
        case deferredToValue

        /// Decode value itself if given closure returns true. Otherwise default logic used.
    ...
    ///
    /// The strategy to use in decoding custom values. Defaults to `.deferredToValue`.
    ///
    open var valueDecodingStrategy: ValueDecodingStrategy = .deferredToValue
...
fileprivate class _DictionaryDecoder: Decoder {
    ...
    fileprivate let options: DatabaseDecoder._Options
    ...
    fileprivate init(referencing container: Any?, at codingPath: [CodingKey] = [], options: DatabaseDecoder._Options) {
...
fileprivate struct _DictionaryDecodingStorage {
    // MARK: Properties
    /// The container stack.
    /// Elements may be any one of the Dictionary types (nil, Number, String, Array, Dictionary).
    ...
    fileprivate var topContainer: Any? {
        ...
        return self.containers.last ?? nil
    }
...
fileprivate struct DictionaryCodingKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    fileprivate init(referencing decoder: _DictionaryDecoder, wrapping container: [String: Any?]) {
        ...
        case .convertFromSnakeCase:
            ...
            self.container = Dictionary(container.map {
                key, value in
                (DatabaseDecoder.KeyDecodingStrategy._convertFromSnakeCase(key), value)
            }, uniquingKeysWith: { (first, _) in first })
        case .custom(let converter):
            self.container = Dictionary(container.map {
                key, value in
                (converter(decoder.codingPath + [DatabaseCodingKey(stringValue: key, intValue: nil)]).stringValue, value)
            }, uniquingKeysWith: { (first, _) in first })
        }
    ...
    private func _errorDescription(of key: CodingKey) -> String {
        ...
        case .convertFromSnakeCase:
            ...
            let converted = DatabaseEncoder.KeyEncodingStrategy._convertToSnakeCase(original)
...
fileprivate struct _DictionaryUnkeyedDecodingContainer: UnkeyedDecodingContainer {
    ...
    public mutating func decodeNil() throws -> Bool {
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(Any?.self,
                                              DecodingError.Context(codingPath: self.decoder
                                                                                    .codingPath + [DatabaseCodingKey(index: self.currentIndex)],
    ...
    public mutating func decode(_ type: Bool.Type) throws -> Bool {
        ...
        self.decoder.codingPath.append(DatabaseCodingKey(index: self.currentIndex))
        defer { self.decoder.codingPath.removeLast() }
...
extension _DictionaryDecoder {
    ...
    fileprivate func unbox(_ value: Any?, as type: Decimal.Type) throws -> Decimal? {
        ...
        if let decimal = value as? Decimal {
            return decimal
        } else if let doubleValue = try self.unbox(value, as: Double.self) {
    ...
    fileprivate func unbox<T: Decodable>(_ value: Any?, as type: T.Type) throws -> T? {
        switch self.options.valueDecodingStrategy {
        case .deferredToValue:
            break
        case .asIs(let helper):
            var value = value
            if helper.useValueAsIs(&value, ofType: T.self) {
                self.storage.push(container: value as? T)
                defer { self.storage.popContainer() }
                return value as? T
            }
        }
